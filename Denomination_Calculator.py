...
def calculator():
    # Validate input first - don't let the app crash on bad input
    try:
        amount = int(E1.get())
    except ValueError:
        messagebox.showerror("Invalid Input", "Please enter a valid whole number amount.")
        return
    ...
    # Clear old results before inserting new ones, so numbers don't stack up
...
def newwindow():
    ...
    # Removed new.mainloop() here - t1.mainloop() below already
    # runs the single event loop for the whole app. A second mainloop()
    # on the Toplevel creates a nested loop that can freeze the main
    # window or cause the app to not close properly.
